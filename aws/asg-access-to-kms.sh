...
## Get the KMS ID
KC=$(/usr/local/bin/aws kms list-aliases | grep -E 'AliasName|AliasArn' | sed -e '/\/aws\// d' | grep AliasArn | wc -l)
if [ $KC -gt 1 ]; then
...
ALIAS=$(/usr/local/bin/aws kms list-aliases | grep -E 'AliasName|AliasArn' | sed -e '/\/aws\// d' | grep AliasArn | awk '{print $2}' | sed -e 's/"//g' -e 's/,//g' | awk -F : '{print $NF}')
ARN=$(/usr/local/bin/aws kms describe-key --key-id $ALIAS --query 'KeyMetadata.Arn' --output text)
